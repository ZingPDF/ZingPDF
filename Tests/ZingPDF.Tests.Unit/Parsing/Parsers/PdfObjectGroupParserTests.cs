...
{
    [Fact]
    public async Task ReturnsParsedObjects_WhenTypesAreIdentified()
    {
        // Arrange
        ...
        var parserResolver = new FakeParserRegistryBuilder()
            ...
            .AddParser(dict, "<<>>".Length)
            .Build();

        var tokenTypes = new Queue<Type?>([typeof(Name), typeof(Dictionary), null]);

        var typeIdentifier = A.Fake<ITokenTypeIdentifier>();
        A.CallTo(() => typeIdentifier.TryIdentifyAsync(A<Stream>._))
            .ReturnsLazily(tokenTypes.Dequeue);
        ...
        result.Get<Name>(0).Should().Be(name);
        result.Get<Dictionary>(1).Should().BeEquivalentTo(dict);
    }

    [Fact]
    public async Task AdvancesStream_AndStopsWhenNoMoreTokens()
    {
        // Arrange
        var stream = "/Name".ToStream();
        Name name = "Name";

        var parserResolver = new FakeParserRegistryBuilder()
            .AddParser(name, bytesToConsume: "/Name".Length)
            .Build();

        var typeIdentifier = A.Fake<ITokenTypeIdentifier>();
        var callCount = 0;
        A.CallTo(() => typeIdentifier.TryIdentifyAsync(A<Stream>._))
            .ReturnsLazily(() => callCount++ == 0 ? typeof(Name) : null);
        ...
        stream.Position.Should().Be(stream.Length); // confirms stream was fully consumed
    }

    [Fact]
    public async Task SkipsByte_IfTokenTypeCannotBeIdentified()
    {
        // Arrange
        var stream = "/Name".ToStream();
        Name name = "Name";
        ...
        var typeIdentifier = A.Fake<ITokenTypeIdentifier>();
        var calls = new Queue<Type?>([null, typeof(Name), null]);
        A.CallTo(() => typeIdentifier.TryIdentifyAsync(A<Stream>._))
            .ReturnsLazily(calls.Dequeue);
    ...
    [Fact]
    public async Task StopsParsing_IfSubParserThrows()
    {
        // Arrange
        var stream = "/Name<<>>".ToStream();
        Name name = "Name";
        ...
        A.CallTo(() => parserResolver.GetParserFor(typeof(Dictionary)))
            .Returns(dictParser);

        var tokenTypes = new Queue<Type?>([typeof(Name), typeof(Dictionary)]);
        var typeIdentifier = A.Fake<ITokenTypeIdentifier>();
        A.CallTo(() => typeIdentifier.TryIdentifyAsync(A<Stream>._))
            .ReturnsLazily(tokenTypes.Dequeue);
    ...
    //    var parserResolver = TestParserResolver
    //        .AddParser<Name>("TEST")
    //        .AddParser(new Dictionary([], A.Dummy<IPdf>(), ObjectOrigin.None))
    //        .Build();
