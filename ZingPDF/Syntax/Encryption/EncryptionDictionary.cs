...
public class EncryptionDictionary : Dictionary
{
    ...
    /// <summary>
    /// <para>(Required) The name of the preferred security handler for this document. It shall be the 
    ...
    /// processor can use any security handler that implements the format specified by SubFilter.</para>
    /// <para>Standard shall be the name of the built-in password-based security handler. Names for other 
    /// security handlers may be registered by using the procedure described in Annex E, "Extending PDF".</para>
    /// </summary>
    public Name Filter => Get<Name>(Constants.DictionaryKeys.Encryption.Filter)!;
    ...
    /// </summary>
    public Name? SubFilter => Get<Name>(Constants.DictionaryKeys.Encryption.SubFilter);

    /// <summary>
    /// <para>(Required) A code specifying the algorithm to be used in encrypting and decrypting the document:</para>
    /// <para>0 An algorithm that is undocumented.This value shall not be used.</para>
    /// <para>1 (Deprecated in PDF 2.0) Indicates the use of 7.6.3.2, "Algorithm 1: Encryption of data using 
    /// the RC4 or AES algorithms" (deprecated in PDF 2.0) with a file encryption key length of 40 bits; see 
    /// below.</para>
    /// <para>2 (PDF 1.4; deprecated in PDF 2.0) Indicates the use of 7.6.3.2, "Algorithm 1: Encryption of data 
    /// using the RC4 or AES algorithms" (deprecated in PDF 2.0) but permitting file encryption key lengths 
    /// greater than 40 bits.</para>
    /// <para>3 (PDF 1.4; deprecated in PDF 2.0) An unpublished algorithm that permits file encryption key lengths 
    /// ranging from 40 to 128 bits.This value shall not appear in a conforming PDF file.</para>
    /// <para>4 (PDF 1.5; deprecated in PDF 2.0) The security handler defines the use of encryption and decryption 
    ...
    /// of 128 bits.</para>
    /// <para>5 (PDF 2.0) The security handler defines the use of encryption and decryption in the document, using 
    /// the rules specified by the CF, StmF, StrF and EFF entries using 7.6.3.3, "Algorithm 1.A: Encryption of data 
    /// using the AES algorithms" with a file encryption key length of 256 bits.</para>
    /// </summary>
    public Integer V => Get<Integer>(Constants.DictionaryKeys.Encryption.V)!;
    ...
    /// </summary>
    public Integer? Length => Get<Integer>(Constants.DictionaryKeys.Encryption.Length);

    /// <summary>
    /// <para>(Optional; meaningful only when the value of V is 4 (PDF 1.5) or 5 (PDF 2.0)) A dictionary whose 
    ...
    /// crypt filter names").</para>
    /// <para>Any keys in the CF dictionary that are listed in "Table 26 — Standard crypt filter names" shall be 
    /// ignored by a PDF processor. Instead, the PDF processor shall use properties of the respective standard 
    /// crypt filters.</para>
    /// </summary>
    public Dictionary? CF => Get<Dictionary>(Constants.DictionaryKeys.Encryption.CF);
    ...
    /// </summary>
    public Name? StmF => Get<Name>(Constants.DictionaryKeys.Encryption.StmF);
    ...
    /// <para>Default value: Identity.</para>
    /// </summary>
    public Name? StrF => Get<Name>(Constants.DictionaryKeys.Encryption.StrF);
