...
public class AsyncMultiProperty<T1, T2>(IPdfObject value)
    where T1 : class, IPdfObject
    where T2 : class, IPdfObject
{
    public async Task<Either<T1, T2>> GetAsync(IIndirectObjectDictionary indirectObjectDictionary)
    {
        if (value is IndirectObjectReference ior)
        {
            var indirectObject = await indirectObjectDictionary.GetAsync(ior)
