...
namespace ZingPDF.Parsing
{
    internal class ObjectFinder
    {
        ...
        public long? Find(Stream stream, string token, bool forwards = true, int limit = 1024)
        {
            if (!stream.CanSeek)
            {
            ...
            return forwards
                ? FindForwards(stream, tokenBytes, limit)
                : FindBackwards(stream, tokenBytes, limit);
        }
        ...
        private static long? FindBackwards(Stream stream, byte[] tokenBytes, int limit)
        {
            ...
            int read = stream.Read(buffer, 0, windowLength);
        ...
        private static long? FindForwards(Stream stream, byte[] tokenBytes, int limit)
        {
            long originalPosition = stream.Position;
            ...
            while (stream.Position < searchEnd)
            {
                ...
                int read = stream.Read(buffer, overlapLength, readSize);
                if (read == 0)
                {
                    break;
                }
