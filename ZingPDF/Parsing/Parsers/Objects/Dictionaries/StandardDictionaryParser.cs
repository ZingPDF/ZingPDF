...
internal class StandardDictionaryParser : DictionaryParser, IParser<Dictionary>
{
    ...
    public async ITask<Dictionary> ParseAsync(Stream stream, ParseContext context)
    {
        ...
        if (dictType is null)
        {
            output = new Dictionary(dict, _pdf, ObjectOrigin.ParsedDocumentObject);
        ...
        switch (dictType)
        {
            case Type t when t == typeof(DocumentCatalogDictionary):
                output = DocumentCatalogDictionary.FromDictionary(dict, _pdf, ObjectOrigin.ParsedDocumentObject);
                goto DictionaryParsed;

            case Type t when t == typeof(PageDictionary):
                output = PageDictionary.FromDictionary(dict, _pdf, ObjectOrigin.ParsedDocumentObject);
                goto DictionaryParsed;

            case Type t when t == typeof(PageTreeNodeDictionary):
                output = PageTreeNodeDictionary.FromDictionary(dict, _pdf, ObjectOrigin.ParsedDocumentObject);
                goto DictionaryParsed;

            case Type t when t == typeof(CrossReferenceStreamDictionary):
                output = CrossReferenceStreamDictionary.FromDictionary(dict, _pdf, ObjectOrigin.ParsedDocumentObject);
                goto DictionaryParsed;

            case Type t when t == typeof(ObjectStreamDictionary):
                output = ObjectStreamDictionary.FromDictionary(dict, _pdf, ObjectOrigin.ParsedDocumentObject);
                goto DictionaryParsed;

            case Type t when t == typeof(AnnotationDictionary):
                output = AnnotationDictionary.FromDictionary(dict, _pdf, ObjectOrigin.ParsedDocumentObject);
                goto DictionaryParsed;

            case Type t when t == typeof(MetadataStreamDictionary):
                output = MetadataStreamDictionary.FromDictionary(dict, _pdf, ObjectOrigin.ParsedDocumentObject);
                goto DictionaryParsed;

            case Type t when t == typeof(WidgetAnnotationDictionary):
                output = WidgetAnnotationDictionary.FromDictionary(dict, _pdf, ObjectOrigin.ParsedDocumentObject);
                goto DictionaryParsed;

            case Type t when t == typeof(FieldDictionary):
                output = FieldDictionary.FromDictionary(dict, _pdf, ObjectOrigin.ParsedDocumentObject);
                goto DictionaryParsed;

            case Type t when t == typeof(Type1FormDictionary):
                output = Type1FormDictionary.FromDictionary(dict, _pdf, ObjectOrigin.ParsedDocumentObject);
                goto DictionaryParsed;

            case Type t when t == typeof(ImageDictionary):
                output = ImageDictionary.FromDictionary(dict, _pdf, ObjectOrigin.ParsedDocumentObject);
                goto DictionaryParsed;

            case Type t when t == typeof(Type1FontDictionary):
                output = Type1FontDictionary.FromDictionary(dict, _pdf, ObjectOrigin.ParsedDocumentObject);
                goto DictionaryParsed;

            case Type t when t == typeof(TrueTypeFontDictionary):
                output = TrueTypeFontDictionary.FromDictionary(dict, _pdf, ObjectOrigin.ParsedDocumentObject);
                goto DictionaryParsed;

            case Type t when t == typeof(Type3FontDictionary):
                output = Type3FontDictionary.FromDictionary(dict, _pdf, ObjectOrigin.ParsedDocumentObject);
                goto DictionaryParsed;

            case Type t when t == typeof(Type0FontDictionary):
                output = Type0FontDictionary.FromDictionary(dict, _pdf, ObjectOrigin.ParsedDocumentObject);
                goto DictionaryParsed;

            case Type t when t == typeof(FontDescriptorDictionary):
                output = FontDescriptorDictionary.FromDictionary(dict, _pdf, ObjectOrigin.ParsedDocumentObject);
                goto DictionaryParsed;

            case Type t when t == typeof(InteractiveFormDictionary):
                output = InteractiveFormDictionary.FromDictionary(dict, _pdf, ObjectOrigin.ParsedDocumentObject);
                goto DictionaryParsed;

            case Type t when t == typeof(LinearizationParameterDictionary):
                output = LinearizationParameterDictionary.FromDictionary(dict, _pdf, ObjectOrigin.ParsedDocumentObject);
                goto DictionaryParsed;

            case Type t when t == typeof(StreamDictionary):
                output = StreamDictionary.FromDictionary(dict, _pdf, ObjectOrigin.ParsedDocumentObject);
                goto DictionaryParsed;

            case Type t when t == typeof(AppearanceDictionary):
                output = AppearanceDictionary.FromDictionary(dict, _pdf, ObjectOrigin.ParsedDocumentObject);
                goto DictionaryParsed;

            case Type t when t == typeof(EncodingDictionary):
                output = EncodingDictionary.FromDictionary(dict, _pdf, ObjectOrigin.ParsedDocumentObject);
                goto DictionaryParsed;

            case Type t when t == typeof(CIDFontDictionary):
                output = CIDFontDictionary.FromDictionary(dict, _pdf, ObjectOrigin.ParsedDocumentObject);
                goto DictionaryParsed;

            case Type t when t == typeof(CIDSystemInfoDictionary):
                output = CIDSystemInfoDictionary.FromDictionary(dict, _pdf, ObjectOrigin.ParsedDocumentObject);
                goto DictionaryParsed;
        }
